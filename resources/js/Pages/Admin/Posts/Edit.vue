<template>
    <AdminLayout>
        <template #title>Éditer "{{ post.title }}"</template>
        ...
        <form class="panel post-form" @submit.prevent="submit">
            ...
            <label class="field">
                ...
                <div class="calories-row">
                    <input v-model.number="form.calories" type="number" min="0" class="input input--sm" />
            ...
            <label v-if="can.publish" class="field">
                <span>Statut</span>
...
export default {
    ...
    props: { post: Object, categories: Array, can: Object },
    data() {
        return {
            form: useForm({
    ...
    methods: {
        ...
        submit() {
            this.form.transform((data) => ({ ...data, _method: 'put' }))
                .post(route('admin.posts.update', this.post.id), { forceFormData: true });
        },
    },
};
</script>
